...
// 	ObjectMapper jsondata = new ObjectMapper();
	...
	public List<Countries> getAllCountries() {
		...
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.COUNTRIES_GET);
...
*/
	...
	public ResponseBean enableCountry(String isocode, boolean status) {
		...
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.COUNTRIES_PUT);
	...
	public List<Countries> getActiveCountries() {
		...
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.COUNTRIES_ACTIVE_GET);
		List<Countries> countries = null;
		//try {
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				restTemplate = new RestTemplate(requestFacotry.validateInventorySsl(urls.getTimeout()));
