...
import com.lc.sk.inventory.security.beans.SubCatDetails;
...
// 	ObjectMapper jsondata = new ObjectMapper();
	...
	public List<SubCategories> getAllSubCategoriesDetails() {
		...
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.SUBCATEGORIES_GET);
...
//		}
	...
	public ResponseBean insertSubcategorytype(String catid, String categoryname) {
		...
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.SUBCATEGORIES_POST);
...
//		}
	...
	public ResponseBean updateSubcategorytype(String subcatid, String catid, String categoryname) {
		...
		key = headerKeyManagement.getKey(ConstantValues.AUTH_HEADER_KEY_DB_ID);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.SUBCATEGORIES_PUT);
		ResponseBean responseBean = null;
	//	try {
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				restTemplate = new RestTemplate(requestFacotry.validateInventorySsl(urls.getTimeout()));
...
//		}
	...
	public List<SubCatDetails> getAllSubCatDetails() {
		RestTemplate restTemplate;
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.SUBCAT_DETAILS);
		List<SubCatDetails> subcategories = null;
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				restTemplate = new RestTemplate(requestFacotry.validateInventorySsl(urls.getTimeout()));
				headers.set(key.getName(), key.getValue());
				...
				response = restTemplate.exchange(urls.getUrl(), urls.getMethod(), request, String.class);
				if (response.getStatusCode() == HttpStatus.ACCEPTED) {
					SubCatDetails[] subcatid = (SubCatDetails[]) JsonToBeanConverter.convert(response.getBody(),
							SubCatDetails[].class);
					subcategories = ArrayToListConverter.convertArrayToList(subcatid);
				}
				else if(response.getStatusCode()==HttpStatus.OK)
