...
import com.lc.sk.inventory.security.beans.Badges;
...
@Component
public class BadgesRestService {
...
// 	ObjectMapper jsondata = new ObjectMapper();
	...
	public List<Badges> getAllBadges() {
		...
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.BADGES_GET);
		...
		//try {
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				...
				if (response.getStatusCode() == HttpStatus.ACCEPTED) {
					Badges[] badge = (Badges[]) JsonToBeanConverter.convert(response.getBody(), Badges[].class);
					badges = ArrayToListConverter.convertArrayToList(badge);
				}
	...
	public Badges getByBadge(String badge) {
		RestTemplate restTemplate;
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.BADGES_GET);
		...
		//try {
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				restTemplate = new RestTemplate(requestFacotry.validateInventorySsl(urls.getTimeout()));
				headers.set(key.getName(), key.getValue());
				urls.setUrl(urls.getUrl() + ConstantValues.SLASH_TAG + badge);
				HttpEntity<Object> request = new HttpEntity<>(headers);
				response = restTemplate.exchange(urls.getUrl(), urls.getMethod(), request, String.class);
				if (response.getStatusCode() == HttpStatus.ACCEPTED) {
					badges = (Badges) JsonToBeanConverter.convert(response.getBody(),
							Badges.class);
				}
				else if(response.getStatusCode()==HttpStatus.OK)
				{
	...
	public ResponseBean insertBadge(String badge, String description) {
		RestTemplate restTemplate;
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.BADGES_POST);
