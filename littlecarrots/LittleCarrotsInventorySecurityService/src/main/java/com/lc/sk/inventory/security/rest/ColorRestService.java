...
// 	ObjectMapper jsondata = new ObjectMapper();
...

	{
		...
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.COLORSERVICE_POST);
	...
	public List<Color> getAllColors() {
		...
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.COLORSERVICE_GET);
	...
	public ResponseBean updateColor(String colorid, String colorname, String hashcode) {
		...
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		ResponseEntity<String> response;
		UrlBean urls = serviceUrlsDao.getAccessUrl(UrlDetails.COLORSERVICE_PUT);
		ResponseBean responseBean = null;
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				restTemplate = new RestTemplate(requestFacotry.validateInventorySsl(urls.getTimeout()));
				headers.set(key.getName(), key.getValue());
